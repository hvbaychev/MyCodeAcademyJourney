...
оценки като параметър и изчислява средната оценка за всички ученици в масива.+ още много условия :D
+ Средна оценка на всеки студент
+ име на най-добрият и най-слабият студент
+ условие ако въведеният резултат е невалиден 1
*/
...
function studentsCalculation($Evaluation){
    ...
    foreach ($Evaluation as $row){           // start from 1 --> [0] -- in this case some name  ["Georgi", .......
        foreach ($row as $col) {            // take all value on row [".......",  5, 6, 2, 3, 4, 2]
            if ($col <= 0){                 // if some number is negative or 0 we cannot give real result
            ...
            if (is_string($col)){              // check if is string 
                ...
                continue;                      // continue iteration from next in this case 5 .....
            }
            else{
                $count++;   
